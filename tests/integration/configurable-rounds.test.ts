...
async function seedAssignedReviewer(
  ...
  const assignment = (await assigned.json()) as { invitePath?: string | null }
  const inviteUrl = new URL(assignment.invitePath ?? '')
...
describe('a reviewer scores against the round scorecard', () => {
  ...
  it('withholds the other names on the proposal too, and shows them when it is not blind', async () => {
    ...
    const reviewer = parseCookieToken(redeemed.headers.get('set-cookie')) ?? ''
...
describe('an organizer nudges the reviewers who are behind', () => {
  ...
  it('mails the reviewer who owes work, and says how much is outstanding', async () => {
    ...
    const mail = (await capturedMessagesForEmail(env.DB, 'behind.reviewer@example.test')).find(
    ...
    expect(mail?.submissionId).toBeNull()
    ...
    expect(mail?.body).toContain('https://www.openevents.engineer/api/public/session?token=')
    const reminderUrl = new URL(mail?.body.match(/https:\/\/\S+/)?.[0]?.replace(/[.,]$/, '') ?? '')
    const redeemed = await app.request(
      `${reminderUrl.pathname}${reminderUrl.search}`,
      undefined,
      bindings(),
    )
    expect(redeemed.status).toBe(303)
    expect(redeemed.headers.get('location')).toBe('/evaluations')
    const reminderCookie = parseCookieToken(redeemed.headers.get('set-cookie'))
    expect(reminderCookie).not.toBeNull()
    expect(
      (
        await app.request(
          '/api/public/evaluations',
          { headers: { cookie: cookieHeader(reminderCookie ?? '') } },
          bindings(),
        )
      ).status,
    ).toBe(200)
