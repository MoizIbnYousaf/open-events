...
/**
 * Perfect Paper Front door type: 48/52 display, not the 20px page-header
 * chrome. The front door has no toolbar strip, so the heading wraps.
 */
  ...
  return (
    <div className="flex flex-wrap items-center gap-2.5 px-5 pb-6 sm:px-10">
      ...
      <Link to="/start" className={`text-xs text-muted-foreground ${linkVariants()}`}>
        Already in? Email me a sign-in link
      </Link>
    </div>
  )
}
    ...
    <FrontFrame>
      <div className="flex max-w-[880px] flex-col gap-4 px-5 pt-12 pb-7 sm:px-10">
        ...
        <p className="max-w-[880px] text-base leading-[26px] text-muted-foreground">
          Call for papers, review, speaker onboarding, and the published agenda. Jump in as
          organizer, reviewer, or speaker — no extra account.
        </p>
      </div>
    </FrontFrame>
  )
