...
export class EmbedService {
  ...
  async list(actor: OrganizerActor, slug: EventSlug, origin: string): Promise<readonly EmbedDto[]> {
    ...
    return rows.map((row) => this.#toDto(row, origin))
  }
  ...
  async create(
    ...
    void actor
    if (!isEmbedKind(input.kind) || !isEmbedFormat(input.format)) {
      throw new ValidationFailedError('Unknown embed type or format', [])
    }
