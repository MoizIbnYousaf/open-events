...
describe('public schedule', () => {
  ...
  it('exports only after a session is starred and scopes the iCal link', async () => {
    ...
    await mountPage('ready')
    await screen.findByRole('heading', { level: 1, name: 'Schedule' })
    expect(screen.getAllByRole('link', { name: 'Add to Google Calendar' })).toHaveLength(1)
    expect(screen.getAllByRole('link', { name: 'Add to Outlook' })).toHaveLength(1)
    expect(screen.getAllByRole('link', { name: 'Download iCalendar file' })).toHaveLength(1)
    expect(screen.queryByRole('link', { name: /my schedule to calendar/i })).not.toBeInTheDocument()
