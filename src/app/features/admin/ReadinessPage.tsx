...
export default function ReadinessPage({ eventSlug }: ReadinessPageProps) {
  ...
  const blockerOptions = [...new Set(rows.flatMap((row) => row.blockers ?? []))].sort()
  const visibleRows = rows
    .filter((row) => filter === 'all' || (row.blockers ?? []).includes(filter))
    .toSorted(
      (left, right) =>
      ...
      {query.isError ? (
        ...
        <div className="grid gap-3">
          ...
          {visibleRows.length === 0 ? (
            <EmptyState title="No readiness work matches that filter" />
          ) : (
            <Table bordered>
