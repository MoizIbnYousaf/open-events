...
describe('reminder routes and audience fan-out', () => {
  ...
  it('acceptance send fans out to owner plus normalized co-speaker exactly once', async () => {
    ...
    expect(rows.map((row) => row.toEmail)).toEqual(['s***@example.test', 's***@example.test'])
    expect(rows.every((row) => !row.body.includes('token='))).toBe(true)
    ...
    const coSpeakerMessage = await latestCapturedBody(env.DB, 'speaker-b@example.test')
    const coSpeakerUrl = new URL(
      coSpeakerMessage?.match(/https:\/\/\S+/)?.[0]?.replace(/[.,]$/, '') ?? '',
    )
    const redeemed = await app.request(
      `${coSpeakerUrl.pathname}${coSpeakerUrl.search}`,
      undefined,
      bindings(),
    )
    expect(redeemed.status).toBe(303)
    expect(redeemed.headers.get('location')).toBe('/portal')
    const coSpeakerCookie = redeemed.headers.get('set-cookie')?.match(/sp_session=([^;]+)/)?.[1]
    expect(coSpeakerCookie).toBeTruthy()
    expect(
      (
        await app.request(
          '/api/public/profile',
          { headers: { cookie: cookieHeader(coSpeakerCookie ?? '') } },
          bindings(),
        )
      ).status,
    ).toBe(200)
