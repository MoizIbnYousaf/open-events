...
/** Generic throttling envelope with a bounded retry hint and no key disclosure. */
export function rateLimitedResponse(context: Context, retryAfter: string): Response {
  context.header('Retry-After', retryAfter)
  context.header('Cache-Control', 'no-store')
  return toErrorResponse(context, 'forbidden', 429)
}
