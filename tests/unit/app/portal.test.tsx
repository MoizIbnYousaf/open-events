...
describe('speaker portal', () => {
  ...
  it('maps a wrong-capability 403 to role-specific organizer-issued link guidance', async () => {
    fetchHandler = () =>
      jsonResponse({ error: { code: 'forbidden', message: 'raw forbidden copy' } }, 403)

    renderPage()

    expect(
      await screen.findByRole('heading', { level: 1, name: 'Speaker portal link required' }),
    ).toBeInTheDocument()
    expect(
      screen.getByText(/ask the event organizer to issue a fresh speaker portal invitation/i),
    ).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /view recovery instructions/i })).toHaveAttribute(
      'href',
      '/start?access=portal',
    )
    expect(document.body.textContent ?? '').not.toContain('raw forbidden copy')
  })
})
