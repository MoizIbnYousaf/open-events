...
beforeEach(async () => {
  ...
  const submitted = await submitProposal(await submitterCookie(env.DB))
    ...
    expect(readiness.submissions).toEqual([
      {
        ...
        percentComplete: 17,
        ready: false,
        missingTaskKinds: ['submit_bio', 'submit_headshot', 'confirm_participation'],
        blockers: ['submit_bio', 'submit_headshot', 'confirm_participation'],
      },
    ])
  })
})
