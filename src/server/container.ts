...
import {
  ...
  getTtlConfig,
  type SubmitterCapabilityRolloutConfig,
} from './env'
...
export interface ServerDeps {
  ...
  readonly emailDelivery: EmailDeliveryService
}
...
export function buildServerDeps(
  ...
  roleAccessTtlMs = 24 * 60 * 60 * 1000,
  capabilityRollout: SubmitterCapabilityRolloutConfig = {
    writerMode: 'purpose',
  ...
  const session = new SessionService(
    ...
    createSha256TokenHasher(),
    createUuidTokenGenerator(),
    createSessionUnitOfWork(db, emailDeliveryConfig),
    clock,
    lastLegacyWriterCutoff,
    { publicAppOrigin, ttlMs: roleAccessTtlMs },
    capabilityRollout,
  )
  ...
  return {
    ...
    submit: new SubmitService(
      ...
      versions,
      content,
      createSubmitUnitOfWork(db, emailDeliveryConfig),
      clock,
    ...
    capturedMessages: new CapturedMessageService(
      createCapturedMessageRepository(db, emailDeliveryConfig),
    ),
    ...
    emailDelivery: new EmailDeliveryService(
      createEmailDeliveryRepository(db),
      emailSender,
      emailDeliveryConfig,
      clock,
    ),
...
export function depsFromContext(context: ServerContext): ServerDeps | null {
  ...
  return db === null
    ...
    : buildServerDeps(
        db,
        getPublicAppOrigin(context),
        getFilesBinding(context),
        getEmailDeliveryConfig(context),
        selectEmailSender(context.env),
        selectOrbyReplyer(context.env),
        getLegacyWriterCutoff(context),
        getTtlConfig(context).submitterTokenMs,
        getSubmitterCapabilityRolloutConfig(context),
      )
}
