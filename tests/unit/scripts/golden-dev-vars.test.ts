...
describe('golden-dev-vars ownership and content helpers', () => {
  ...
  it('devVarsContent emits the exact sentinel line and all local vars', () => {
    ...
    expect(content).toContain('DEPLOY_ENVIRONMENT=local')
    expect(content).toContain('BUILD_REVISION=local')
    expect(content).toContain('RESOURCE_D1_ID=local-d1')
    expect(content).toContain('RESOURCE_R2_NAME=local-r2')
    expect(content).toContain('PUBLIC_APP_URL=http://localhost:4173')
    expect(content).toContain('SUBMITTER_CAPABILITY_WRITER_MODE=purpose')
    expect(content).toContain('SUBMITTER_CAPABILITY_LEGACY_READER_MODE=bounded')
    expect(content).toContain('RATE_LIMIT_ENVIRONMENT=local')
    expect(content).toContain('EMAIL_DELIVERY_MODE=capture')
    expect(content).toContain('EMAIL_PAYLOAD_KEY_VERSION=v1')
    expect(content).toContain('EMAIL_PAYLOAD_KEY_V1=BwcHBwcHBwcHBwcHBwcHBwcHBwcHBwcHBwcHBwcHBwc=')
