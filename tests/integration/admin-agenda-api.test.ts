...
async function submitFreshProposal(title: string): Promise<string> {
  const cfpCookie = await submitterCookie(env.DB)
  return (await submitProposal(cfpCookie, title)).id
}
...
beforeEach(async () => {
  ...
  const submitted = await submitProposal(await submitterCookie(env.DB), 'Workshop proposal')
...
describe('PUT /api/admin/events/:slug/agenda/:submissionId', () => {
  ...
  it('reports the room double booking it just created', async () => {
    const second = await submitFreshProposal('Second proposal')
  ...
  it('rejects a submission that has not been accepted', async () => {
    const pending = await submitFreshProposal('Not accepted yet')
...
describe('POST /api/admin/events/:slug/agenda/auto-place', () => {
  ...
  it('schedules the unplaced sessions and reports what it did', async () => {
    const second = await submitFreshProposal('Also unplaced')
    await acceptSubmission(second)
...
describe('POST /api/admin/events/:slug/agenda/publish', () => {
  it('publishes the scheduled sessions and leaves the unplaced ones alone', async () => {
    const unplaced = await submitFreshProposal('Still unplaced')
