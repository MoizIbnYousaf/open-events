...
describe('e2e server env', () => {
  it('clears optional browser auth/challenge keys so .env.local cannot alter the gate', () => {
    const env = e2eServerEnv({
      VITE_CLERK_PUBLISHABLE_KEY: 'pk_test_not-used',
      VITE_CLERK_ORGANIZER_POLICY_CONFIGURED: 'true',
      VITE_TURNSTILE_SITE_KEY: 'site-key-not-used',
      PATH: '/bin',
    })
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBe('')
    expect(env.VITE_CLERK_ORGANIZER_POLICY_CONFIGURED).toBe('')
    expect(env.VITE_TURNSTILE_SITE_KEY).toBe('')
