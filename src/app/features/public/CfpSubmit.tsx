...
export default function CfpSubmit({ onSubmitted, onDenied }: CfpSubmitProps) {
  ...
  useEffect(() => {
    if (!submitted || redirectCancelled) return
    const reducedMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches === true
    const redirect = window.setTimeout(() => window.location.assign('/portal'), 8_000)
    if (reducedMotion) return () => window.clearTimeout(redirect)
      ...
      <Card className="mx-auto w-full max-w-[47rem]">
        <CardContent className="grid justify-items-center gap-2 py-8 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            {redirectCancelled
              ? 'Automatic continuation cancelled. Your proposal is safely submitted.'
              : `Continuing to your speaker portal in ${redirectSeconds} seconds.`}
          </p>
          <div className="mt-1 flex flex-wrap justify-center gap-2">
            <TextLink href="/portal" className="min-h-8">
              Continue to speaker portal now
            </TextLink>
            {!redirectCancelled ? (
              <Button type="button" variant="outline" onClick={() => setRedirectCancelled(true)}>
                Stay here
              </Button>
            ) : null}
          </div>
