...
import {
  SubmitterAccessRecoveryError,
  type SubmitterAccessRecoveryKind,
...
function accessRecoveryPath(kind: SubmitterAccessRecoveryKind): string {
  return `/start?access=${encodeURIComponent(kind)}`
}

function accessRecoveryResponse(kind: SubmitterAccessRecoveryKind): Response {
...
export async function handleStart(context: ServerContext): Promise<Response> {
  ...
  // Capture mode intentionally has no provider delivery. Keep the generic
  // anti-enumeration response, but never tell a visitor to wait for an email
  // that this deployment cannot send.
  const acceptedResponse =
    context.env.EMAIL_DELIVERY_MODE === 'capture' ? CAPTURE_START_RESPONSE : START_RESPONSE
  const accepted = () => context.json(acceptedResponse, 202)
  const now = deps.clock.now()
  const source = clientAddress(context)
  const budget = await startMailBudgetReservation(context, normalizeEmail(email), now)
  const sourceKey = source === null ? null : await durableLimitKey(context, 'start-source', source)
  if (budget === null || (source !== null && sourceKey === null)) return accepted()
  const recipientBurstAllowed = await consumeEdgeLimit(
    context,
    'START_RECIPIENT_RATE_LIMITER',
    budget.recipientKey,
  )
  const sourceBurstAllowed =
    sourceKey === null || (await consumeEdgeLimit(context, 'START_SOURCE_RATE_LIMITER', sourceKey))
  if (!recipientBurstAllowed || !sourceBurstAllowed) return accepted()
  const [recipientAllowed, sourceAllowed] = await Promise.all([
    consumeDurableLimit(context.env.DB, {
      scope: 'start_recipient_attempt',
      keyHash: budget.recipientKey,
      now,
      windowSeconds: EDGE_LIMIT_POLICIES.startRecipient.productWindowSeconds,
      limit: EDGE_LIMIT_POLICIES.startRecipient.productLimit,
    }),
    sourceKey === null
      ...
      : consumeDurableLimit(context.env.DB, {
          ...
          keyHash: sourceKey,
          now,
          windowSeconds: EDGE_LIMIT_POLICIES.startSource.productWindowSeconds,
          limit: EDGE_LIMIT_POLICIES.startSource.productLimit,
        }),
  ])
  if (!recipientAllowed || !sourceAllowed) return accepted()
  const remoteAddress = turnstileRemoteAddress(context.req.header('CF-Connecting-IP') ?? '')
  const human = await verifyTurnstile({
    token: typeof body.turnstileToken === 'string' ? body.turnstileToken : '',
    secret: turnstileSecretKey(context),
    ...(remoteAddress === undefined ? {} : { remoteAddress }),
    expectedAction: TURNSTILE_PUBLIC_START_ACTION,
    expectedHostnames: turnstileHostnames(context),
  })
  if (!human) return accepted()
  const input: StartInput = { email, eventSlug, formSlug }
  const ttlMs = getTtlConfig(context).submitterTokenMs
  await deps.session.startSubmitter(input, ttlMs, getPublicAppOrigin(context), budget)
  return accepted()
}
...
export async function handleSessionExchange(context: ServerContext): Promise<Response> {
  ...
  const now = deps.clock.now()
  const source = clientAddress(context)
  const sourceKey = source === null ? null : await durableLimitKey(context, 'redeem-source', source)
  const tokenKey = await durableLimitKey(context, 'redeem-token', token || 'missing')
  if ((source !== null && sourceKey === null) || tokenKey === null) {
    return rateLimitedResponse(context, boundedRetryAfter(60, 300))
  }
  const [sourceBurstAllowed, tokenBurstAllowed] = await Promise.all([
    sourceKey === null
      ? Promise.resolve(true)
      : consumeEdgeLimit(context, 'TOKEN_REDEEM_SOURCE_RATE_LIMITER', sourceKey),
    consumeEdgeLimit(context, 'TOKEN_REDEEM_TOKEN_RATE_LIMITER', tokenKey),
  ])
  if (!sourceBurstAllowed || !tokenBurstAllowed) {
    return rateLimitedResponse(context, boundedRetryAfter(60, 300))
  }
  const sourceAllowed =
    sourceKey === null ||
    (await consumeDurableLimit(context.env.DB, {
      scope: 'redeem_source',
      keyHash: sourceKey,
      now,
      windowSeconds: EDGE_LIMIT_POLICIES.redeemSource.productWindowSeconds,
      limit: EDGE_LIMIT_POLICIES.redeemSource.productLimit,
    }))
  const tokenAllowed = await consumeDurableLimit(context.env.DB, {
    scope: 'redeem_token',
    keyHash: tokenKey,
    now,
    windowSeconds: EDGE_LIMIT_POLICIES.redeemToken.productWindowSeconds,
    limit: EDGE_LIMIT_POLICIES.redeemToken.productLimit,
  })
  if (!sourceAllowed || !tokenAllowed) {
    const [sourceState, tokenState] = await Promise.all([
      sourceKey === null
        ? Promise.resolve({ blocked: false, retryAfterSeconds: 1 })
        : durableLimitState(context.env.DB, {
            scope: 'redeem_source',
            keyHash: sourceKey,
            now,
            windowSeconds: EDGE_LIMIT_POLICIES.redeemSource.productWindowSeconds,
    ...
    return rateLimitedResponse(
      context,
      boundedRetryAfter(
        Math.max(sourceState.retryAfterSeconds, tokenState.retryAfterSeconds),
        EDGE_LIMIT_POLICIES.redeemSource.productWindowSeconds,
      ),
    )
  }
  if (token.length === 0) return accessRecoveryResponse('invalid')
  const ttlMs = getTtlConfig(context).submitterSessionMs
  let result
  try {
    result = await deps.session.redeemSubmitterToken(token, ttlMs)
  } catch (error) {
    if (error instanceof SubmitterAccessRecoveryError) {
      return accessRecoveryResponse(error.recoveryKind)
    }
    throw error
  }
...
export async function handleSubmit(context: ServerContext): Promise<Response> {
  ...
  const rawSession = readSessionToken(context)
  if (rawSession === null) return forbiddenResponse(context)
  const authorization = await deps.session.authorizeCfpSubmit(
    rawSession,
    input,
    getTtlConfig(context).submitterSessionMs,
  )
  const result = await deps.submit.submitWithHandoff(authorization, input)
  const secure = new URL(context.req.url).protocol === 'https:'
  context.header(
    'Set-Cookie',
    serializeSessionCookie(
      result.portalToken,
      sessionCookieMaxAgeSeconds(result.portalExpiresAt, deps.clock.now()),
      secure,
    ),
  )
  return context.json(result.submission)
}
...
export async function handleAddOwnFileComment(context: ServerContext): Promise<Response> {
  ...
  return context.json(await deps.contentLibrary.addOwnComment(actor, kind, body.body), 201)
}
...
export async function handleListOwnFileVersions(context: ServerContext): Promise<Response> {
  ...
  const kind = context.req.param('kind')
  if (kind !== 'document' && kind !== 'headshot') return validationFailedResponse(context)
  return context.json(await deps.contentLibrary.listOwnVersions(actor, kind))
}
...
export async function handleListOwnFileComments(context: ServerContext): Promise<Response> {
  ...
  const kind = context.req.param('kind')
  if (kind !== 'document' && kind !== 'headshot') return validationFailedResponse(context)
  return context.json(await deps.contentLibrary.listOwnComments(actor, kind))
}
...
export async function handleGetPublicEmbed(context: ServerContext): Promise<Response> {
  ...
  const origin = getPublicAppOrigin(context)
  const dataUrl = `${origin}/api/public/events/${event.slug}/schedule`
  const speakersUrl = `${origin}/api/public/events/${event.slug}/speakers`
  if (embed.kind === 'sessions' && embed.format === 'json') {
    const response = await handleGetPublicSchedule(withEventSlug(context, event.slug))
    if (response.status !== 200 || embed.trackFilter === '') return response
    const schedule = (await response.json()) as {
      readonly timezone: string
      readonly sessions: readonly { readonly track: string }[]
    }
    return context.json(
      {
        ...schedule,
        sessions: schedule.sessions.filter((session) => session.track === embed.trackFilter),
      },
      200,
      { 'Cache-Control': 'public, max-age=60' },
    )
  }
  if (embed.kind === 'speakers' && embed.format === 'json') {
    return handleGetPublicSpeakers(withEventSlug(context, event.slug))
  }
...
function withEventSlug(context: ServerContext, slug: string): ServerContext {
...
export function registerPublicRoutes(app: Hono<ServerEnv>): void {
  ...
  app.get(
    ...
    requireSession(),
    requireActor('submitter'),
    requireCapability('cfp'),
    handleGetActiveDraft,
  )
  app.get(
    '/api/public/draft/:id',
    requireSession(),
  ...
  app.post('/api/public/submit', csrfGate(), handleSubmit)
  app.get(
    '/api/public/tasks',
    requireSession(),
    requireActor('submitter'),
    requireCapability('portal'),
    handleListSpeakerTasks,
  )
  app.get(
