...
describe('the organizer message log', () => {
  ...
  it('shows what the event has sent, newest first', async () => {
    ...
    expect(entries[0]?.toEmail).toBe('s***@example.test')
    expect(entries[0]?.deliveryStatus).toBe('captured')
  })

  it('never exposes a bearer link or full recipient through the organizer API', async () => {
    const organizer = await organizerCookie()
    await requestSignInLink('ada@example.test')

    const entry = (await log(organizer)).find((row) => row.toEmail === 'a***@example.test')

    expect(String(entry?.body)).not.toContain('token=')
    expect(String(entry?.body)).toContain('protected')
