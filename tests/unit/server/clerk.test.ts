...
import {
  ...
  isAuthorizedClerkOrganizer,
  verifyClerkSessionToken,
} from '../../../src/server/clerk'
...
describe('isAuthorizedClerkOrganizer', () => {
  it('denies by default and accepts only an explicitly allowlisted user id', () => {
    expect(isAuthorizedClerkOrganizer({ userId: 'user_abc' }, [])).toBe(false)
    expect(isAuthorizedClerkOrganizer({ userId: 'user_abc' }, ['user_other'])).toBe(false)
    expect(isAuthorizedClerkOrganizer({ userId: 'user_abc' }, ['user_other', 'user_abc'])).toBe(
      true,
    )
  })
})
