  ...
  it('shows a newly invited reviewer on the roster immediately', async () => {
    ...
    expect(
      await env.DB.prepare(
        ...
        .bind(seated?.contactId)
        .first(),
    ).toEqual({ purpose: 'evaluation', form_id: null })
  })

  it('does not return a stale invite or claim delivery when the recipient cooldown limits a retry', async () => {
    const organizer = await organizerCookie()
    const first = await invite(organizer, COLD_REVIEWER, 'Cold Reviewer')
    expect(first.status).toBe(200)
    const firstBody = (await first.json()) as { invitePath: string | null; inviteSent: boolean }
    expect(firstBody.inviteSent).toBe(true)
    expect(firstBody.invitePath).toMatch(
      /^https:\/\/www\.openevents\.engineer\/api\/public\/session\?token=/,
    )

    const retry = await invite(organizer, COLD_REVIEWER, 'Cold Reviewer')
    expect(retry.status).toBe(200)
    expect(await retry.json()).toMatchObject({ invitePath: null, inviteSent: false })
    expect(await capturedMessagesForEmail(env.DB, COLD_REVIEWER)).toHaveLength(1)
  })

  it('does not reuse a historical invite when the environment mail cap is full', async () => {
    const organizer = await organizerCookie()
    ...
    await env.DB.prepare(
      `WITH RECURSIVE counter(value) AS (
       ...
       SELECT 'reviewer-env-cap-' || value,
              'v1:start-recipient:' || printf('%064x', value), ?, ?
       FROM counter`,
    )
      .bind(environmentKey, now)
      .run()
    await env.DB.prepare(
      ...
      .bind(
        'historical-reviewer-invite',
        DEMO_CONF_2026_ID,
        'environment.capped@example.test',
        'Historical invite',
        'Old /api/public/session?token=stale-token',
        now,
      )
      .run()
    ...
    expect(response.status).toBe(200)
    expect(await response.json()).toMatchObject({ invitePath: null, inviteSent: false })
  })
...
describe('a seated reviewer signing in lands on their queue', () => {
  async function startAndFollow(email: string): Promise<string | null> {
    await env.DB.prepare(
  ...
  async function followInvite(accessUrl: string): Promise<{
    readonly location: string | null
    readonly cookie: string
  }> {
    const inviteUrl = new URL(accessUrl)
    const redeem = await app.request(
    ...
    expect(redeem.status).toBe(303)
    return {
      location: redeem.headers.get('location'),
      cookie: parseCookieToken(redeem.headers.get('set-cookie')) ?? '',
    }
  }

  it('sends a reviewer the organizer just invited to the review queue', async () => {
    const organizer = await organizerCookie()
    const invited = await invite(organizer, COLD_REVIEWER, 'Cold Reviewer')
    expect(invited.status).toBe(200)
    const body = (await invited.json()) as { invitePath: string }

    expect((await followInvite(body.invitePath)).location).toBe('/evaluations')
  })

  it('still sends everybody else to the call for papers', async () => {
    expect(await startAndFollow('not.a.reviewer@example.test')).not.toBe('/evaluations')
  })

  it('revokes review-queue access when the seat is removed', async () => {
    const organizer = await organizerCookie()
    const seated = (await (await invite(organizer, COLD_REVIEWER)).json()) as {
      contactId: string
      invitePath: string
    }
    const reviewer = await followInvite(seated.invitePath)
    ...
    const denied = await app.request(
      ...
      { headers: { cookie: cookieHeader(reviewer.cookie) } },
      bindings(),
    )
...
async function reviewerCookie(accessUrl: string): Promise<string> {
  const inviteUrl = new URL(accessUrl)
  const redeem = await app.request(
    `${inviteUrl.pathname}${inviteUrl.search}`,
    undefined,
...
describe('an organizer removes a reviewer from the committee', () => {
  ...
  it('keeps the assignments and scores of a reviewer who had done work', async () => {
    ...
    const reviewer = await reviewerCookie(seated.invitePath)
