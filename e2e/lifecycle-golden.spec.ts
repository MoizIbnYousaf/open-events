...
const SPEAKER_EMAIL = liveTestEmail('lifecycle-speaker')
const SPEAKER_B_EMAIL = liveTestEmail('lifecycle-speaker-b')
const CO_SPEAKER_EMAIL = liveTestEmail('lifecycle-cospeaker')
const EVALUATOR_EMAIL = liveTestEmail('reviewer.one')
...
const ROW_LINK = (title: string, email = SPEAKER_EMAIL) => `${title} — ${email}`
...
const EXPECTED_MUTATIONS: Readonly<Record<string, number>> = {
  'POST /api/public/start': 2, // one per speaker proposal
  ...
  [`POST /api/admin/events/${EVENT_SLUG}/evaluations/committee`]: 1,
...
test('golden lifecycle: configure, submit, evaluate, accept, onboard, communicate, schedule, publish', async ({
  ...
  try {
    ...
    await startProposal(TITLE, SPEAKER_EMAIL, true)
    await startProposal(TITLE_B, SPEAKER_B_EMAIL, false)
    ...
    await expect(
      adminPage.getByRole('link', { name: ROW_LINK(TITLE_B, SPEAKER_B_EMAIL) }),
    ).toHaveCount(1)
    ...
    // Seat the evaluator first. This creates the contact and one purpose-bound
    // evaluation invitation without borrowing the public CFP start path.
    await adminPage.getByLabel('Reviewer email').fill(EVALUATOR_EMAIL)
    await adminPage.getByRole('button', { name: 'Add reviewer' }).click()
    ...
    const evaluatorPath = evaluatorLinks
      .filter((message) => message.body.includes('Open your private evaluation queue:'))
      .flatMap(
        (message) => message.body.match(/https?:\/\/\S+\/api\/public\/session\?token=\S+/) ?? [],
      )
      .at(-1)
    ...
    await adminPage.goto(`/admin/events/${EVENT_SLUG}/submissions`)
    await adminPage.getByRole('link', { name: ROW_LINK(TITLE_B, SPEAKER_B_EMAIL) }).click()
    await adminPage.getByRole('button', { name: 'Accept proposal' }).click()
    await adminPage.getByRole('button', { name: 'Confirm acceptance' }).click()
    await expect(adminPage.getByText('Accepted', { exact: true }).first()).toBeVisible()

    // The featured speaker enters from the real per-recipient acceptance link,
    // not by retaining a pre-acceptance CFP cookie. This also restores the
    // featured speaker context after the conflict partner used the same page.
    await adminPage.goto(`/admin/events/${EVENT_SLUG}/submissions/${submissionId}`)
    ...
    if (process.env.LIVE_ALLOW_MUTATION === 'acceptance') {
      ...
      expect(captured.status()).toBe(200)
      recordResponse(captured, 'GET')
      featuredInbox = (await captured.json()) as Array<{ body: string }>
    }
    const portalLink = featuredInbox
      .flatMap(
        (message) => message.body.match(/https?:\/\/\S+\/api\/public\/session\?token=\S+/) ?? [],
      )
      .at(-1)
    expect(portalLink, 'acceptance message contains a portal capability link').toBeDefined()
    await speakerPage.goto(portalLink ?? '')
    ...
    // Distinct owners in the same room and time create exactly one
    // deterministic room conflict.
    await expect(adminPage.getByText('1 conflict to resolve.')).toBeVisible()
