...
async function speakerWithSubmission(): Promise<{ cookie: string; submissionId: string }> {
  ...
  return { cookie: portalCookie, submissionId: body.id }
}
...
describe('a cold-invited reviewer signs in and finds their queue', () => {
  it('reaches the review surface through the ordinary magic link', async () => {
    ...
    const organizer = await organizerCookie()
    const invited = await inviteReviewer(organizer, {
      email: COLD_REVIEWER,
      name: 'Cold Reviewer',
    })
    expect(invited.status).toBe(200)
    const invitation = (await invited.json()) as CommitteeMemberBody
    ...
    const { cookie, location } = await reviewerSignIn(invitation.invitePath)
  ...
  it('reports an empty queue rather than a refusal before any assignment', async () => {
    const organizer = await organizerCookie()
    const invited = await inviteReviewer(organizer, { email: COLD_REVIEWER })
    expect(invited.status).toBe(200)
    const invitation = (await invited.json()) as CommitteeMemberBody
    const { cookie } = await reviewerSignIn(invitation.invitePath)
    const queue = await app.request(
      '/api/public/evaluations',
      { headers: { cookie: cookieHeader(cookie) } },
      bindings(),
...
describe('provisioning reuses an identity and never rewrites it', () => {
  ...
  it('grants a seated speaker no reach into anyone else’s proposals', async () => {
    ...
    const { cookie } = await reviewerSignIn(member.invitePath)
    const queue = await app.request(
      '/api/public/evaluations',
      { headers: { cookie: cookieHeader(cookie) } },
      bindings(),
    ...
    // Evaluation capability cannot enter the speaker-portal service at all.
...
describe('an organizer can read what the committee actually said', () => {
  it('carries each reviewer rating and comment, and says who has not answered', async () => {
    const { submissionId } = await speakerWithSubmission()
    const organizer = await organizerCookie()
    const reviewerInvites = new Map<string, string>()
    for (const email of ['reviewer.one@example.test', 'reviewer.two@example.test']) {
      ...
      expect(assigned.status).toBe(200)
      const body = (await assigned.json()) as { invitePath?: string | null }
      if (body.invitePath !== undefined && body.invitePath !== null) {
        reviewerInvites.set(email, body.invitePath)
      }
    }

    // Only reviewer one answers.
    const reviewer = (await reviewerSignIn(reviewerInvites.get('reviewer.one@example.test') ?? ''))
      .cookie
  ...
  it('never lets a reviewer read another reviewer’s words', async () => {
    ...
    const one = (await reviewerSignIn(reviewerInvites.get('reviewer.one@example.test') ?? ''))
      .cookie
    await app.request(
      '/api/public/evaluations',
      {
    ...
    const two = (await reviewerSignIn(reviewerInvites.get('reviewer.two@example.test') ?? ''))
      .cookie
    const queue = await app.request(
      '/api/public/evaluations',
