...
import { ALLOWED_ORIGIN, bindings, cookieHeader, submitterCookie } from './m2c-helpers'
...
describe('public headshot uploads (R2 + D1)', () => {
  ...
  it('never serves another submitter headshot', async () => {
    const owner = await submitterCookie(env.DB)
    await putHeadshot(owner, pixels(64))
    const other = await submitterCookie(env.DB, {}, 'speaker-b@example.test')
