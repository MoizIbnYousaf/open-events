...
import { isAuthorizedClerkOrganizer, readBearerToken, verifyClerkSessionToken } from '../clerk'
import { csrfGate } from '../csrf'
import {
  clerkOrganizerUserIds,
  clerkPublishableKey,
  clerkSecretKey,
...
export async function handleAdminSession(context: ServerContext): Promise<Response> {
  ...
  const sourceKey =
    source === null ? null : await durableLimitKey(context, 'admin-login-source', source)
  if (source !== null && sourceKey === null) {
    return rateLimitedResponse(
      context,
      boundedRetryAfter(60, EDGE_LIMIT_POLICIES.adminLogin.productWindowSeconds),
    )
  }
  const state =
    sourceKey === null
      ? { blocked: false, retryAfterSeconds: 1 }
      : await durableLimitState(context.env.DB, {
          scope: 'admin_login_failure',
          keyHash: sourceKey,
          now,
          windowSeconds: EDGE_LIMIT_POLICIES.adminLogin.productWindowSeconds,
          limit: EDGE_LIMIT_POLICIES.adminLogin.productLimit,
        })
  if (state.blocked) {
    return rateLimitedResponse(
      context,
      boundedRetryAfter(
        ...
        EDGE_LIMIT_POLICIES.adminLogin.productWindowSeconds,
      ),
    )
  }
  if (
    sourceKey !== null &&
    !(await consumeEdgeLimit(context, 'ADMIN_LOGIN_RATE_LIMITER', sourceKey))
  ) {
    return rateLimitedResponse(
      context,
      boundedRetryAfter(60, EDGE_LIMIT_POLICIES.adminLogin.productWindowSeconds),
  ...
  let result
  try {
    result = await deps.session.organizerLogin(secret, expected, ttlMs)
  } catch (error) {
    if (error instanceof ApplicationError && error.code === 'unauthorized') {
      if (sourceKey === null) throw error
      ...
      const afterFailure = await durableLimitState(context.env.DB, {
        scope: 'admin_login_failure',
        keyHash: sourceKey,
        now,
        windowSeconds: EDGE_LIMIT_POLICIES.adminLogin.productWindowSeconds,
      ...
      if (afterFailure.blocked) {
        return rateLimitedResponse(
          context,
          boundedRetryAfter(
            afterFailure.retryAfterSeconds,
            EDGE_LIMIT_POLICIES.adminLogin.productWindowSeconds,
          ),
        )
      }
  ...
  if (sourceKey !== null) {
    await clearDurableLimit(context.env.DB, 'admin_login_failure', sourceKey)
  }
...
export async function handleAdminClerkSession(context: ServerContext): Promise<Response> {
  ...
  const allowedUserIds = clerkOrganizerUserIds(context)
  if ((publishableKey.length === 0 && secretKey.length === 0) || allowedUserIds.length === 0) {
  ...
  if (identity === null || !isAuthorizedClerkOrganizer(identity, allowedUserIds)) {
    return unauthorizedResponse(context)
  }
  const ttlMs = getTtlConfig(context).organizerSessionMs
...
export async function handleInviteSpeaker(context: ServerContext): Promise<Response> {
  ...
  const result = await deps.session.issueRoleAccess(actor, {
    ...
    renderBody: (accessUrl) => `Open your private speaker portal link: ${accessUrl}`,
    kind: 'confirmation',
    submissionId: null,
    proof: { kind: 'speaker-member', submissionId: null },
  })
  if (result.outcome === 'limited') {
    return context.json({ sent: false, to: person.email, invitePath: null })
  }
  return context.json({
    sent: true,
    to: person.email,
    invitePath: result.accessUrl,
  })
}
...
export async function handleAddCommitteeMember(context: ServerContext): Promise<Response> {
  ...
  const invite = await sendReviewerInvite(context, deps, actor, eventId, seated.contactId, email)
...
export async function handleAssignEvaluator(context: ServerContext): Promise<Response> {
  ...
  const invite = await sendReviewerInvite(
    context,
    deps,
    actor,
    eventId,
...
export function registerAdminRoutes(app: Hono<ServerEnv>): void {
  ...
  app.post(
    ...
    requireSession(),
    requireActor('organizer'),
    organizerSendLimit({ skipPreview: true }),
