...
function buildHarness(clockNow: string = FIXED_NOW) {
  ...
  return { service, unitOfWork, tokens, sessions, contacts, messages, tokenCalls }
}
...
describe('SessionService.startSubmitter', () => {
  it('normalizes and dedupes the contact, persists the token with event context, and captures the link', async () => {
    ...
    if (response.outcome !== 'issued') throw new Error('expected an issued start')
    expect(response.message.toEmail).toBe('speaker-a@example.test')
    ...
    expect(message?.body).toContain('http://localhost/api/public/session?token=token-1')
  })

  it('preserves a limited internal outcome without claiming a new message', async () => {
    const { service, unitOfWork, tokens, contacts, messages } = buildHarness()
    unitOfWork.issueStartResult = { outcome: 'limited' }

    await expect(
      service.startSubmitter(startInput(), 60_000, 'http://localhost', startMailBudgetFixture()),
    ).resolves.toEqual({ outcome: 'limited' })
  ...
  it('rejects unknown events and unpublished forms', async () => {
    ...
    await expect(
      service.startSubmitter(
        startInput({ eventSlug: 'unknown-event' }),
        60_000,
        'http://localhost',
        startMailBudgetFixture(),
      ),
    ...
    await expect(
      service.startSubmitter(
        startInput({ formSlug: 'unknown-form' }),
        60_000,
        'http://localhost',
        startMailBudgetFixture(),
      ),
  ...
  it('rejects an invalid email and invalid TTLs', async () => {
    ...
    await expect(
      service.startSubmitter(startInput(), 0, 'http://localhost', startMailBudgetFixture()),
