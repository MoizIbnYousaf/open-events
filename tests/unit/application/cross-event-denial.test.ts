...
describe('cross-event denial for SubmitterActor', () => {
  ...
  it('denies submits and submission reads across events with zero writes', async () => {
    ...
    expect(await submitService.getOwnDetail(crossEventActor, victim.id)).toBeNull()
    expect(await submitService.getOwnDetail(ownerActor, victim.id)).toMatchObject({ id: victim.id })
  })
