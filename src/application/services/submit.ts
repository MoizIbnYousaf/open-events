...
export class SubmitService {
  ...
  /**
   * Public CFP submit plus portal handoff. The application accepts retry
   * authority only from SessionService and the adapter re-proves the exact
   * session/draft/request tuple against persisted handoff state.
   */
  async submitWithHandoff(
    authorization: CfpSubmitAuthorization,
    input: SubmitInput,
  ): Promise<{
    readonly submission: SubmissionDetailDto
    ...
    if (authorization.mode === 'retry') {
      const recovered = await this.#submitUnitOfWork.recoverHandoff({
        cfpSessionId: authorization.cfpSessionId,
        eventId: authorization.actor.eventId,
        ownerContactId: authorization.actor.contactId,
        originDraftId: input.originDraftId,
        requestHash: authorization.requestHash,
      })
      if (recovered.outcome === 'handoff-invalid') {
        throw new ApplicationError('forbidden', 'Submit handoff retry was not recognized')
      }
      return {
        submission: await this.#detail(recovered.submission),
        portalToken: authorization.portalToken,
        portalExpiresAt: recovered.handoff.expiresAt,
      }
    }
    const result = await this.#submit(authorization.actor, input, {
      cfpSessionId: authorization.cfpSessionId,
      requestHash: authorization.requestHash,
      portalSession: authorization.portalSession,
      source: authorization.source,
    })
    if (result.handoff === undefined) {
      throw new Error('public submit completed without a portal handoff')
    }
    return {
      submission: result.detail,
      portalToken: authorization.portalToken,
      portalExpiresAt: result.handoff.expiresAt,
    }
  }
  ...
  async #submit(
    ...
    input: SubmitInput,
    handoff?: import('../ports/submit-unit-of-work').SubmitSessionHandoffIntent,
  ): Promise<{
    readonly detail: SubmissionDetailDto
    readonly handoff?: import('../ports/submit-unit-of-work').SubmitSessionHandoffResult
  }> {
    ...
    if (existing !== null) {
      ...
      if (handoff !== undefined) {
        const recovered = await this.#submitUnitOfWork.recoverHandoff({
          cfpSessionId: handoff.cfpSessionId,
        ...
        return { detail: await this.#detail(recovered.submission), handoff: recovered.handoff }
      }
      return { detail: await this.#detail(existing) }
    }
    ...
    switch (result.outcome) {
      case 'inserted':
        return {
          detail: await this.#detail(result.submission),
