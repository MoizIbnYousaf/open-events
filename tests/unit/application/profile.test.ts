...
describe('getOwnProfile', () => {
  ...
  it.each(['cfp', 'evaluation'] as const)(
    'rejects the %s capability before reading speaker data',
    async (capability) => {
      await expect(
        service.getOwnProfile(createSubmitterActor({ capability })),
      ).rejects.toMatchObject({ code: 'forbidden' })
    },
  )

  it('rejects a fabricated null-capability actor before reading speaker data', async () => {
    const rawNullActor = {
    ...
    await expect(service.getOwnProfile(rawNullActor)).rejects.toMatchObject({ code: 'forbidden' })
  })
})
