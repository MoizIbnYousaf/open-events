...
export class InMemoryCapturedMessageRepository implements CapturedMessageRepository {
  ...
  async findAccessBoundBySubmissionKindEmail(
    submissionId: string,
    kind: CapturedMessage['kind'],
    toEmail: string,
  ): Promise<CapturedMessage | null> {
    return this.findBySubmissionKindEmail(submissionId, kind, toEmail)
  }
...
export class InMemoryObjectStorage implements ObjectStoragePort {
  ...
  async listPrefix(prefix: string, cursor?: string) {
    const keys = [...this.objects.keys()].filter((key) => key.startsWith(prefix)).sort()
    const offset = cursor === undefined ? 0 : Number(cursor)
    const page = keys.slice(offset, offset + 2)
    const next = offset + page.length
    return { keys: page, cursor: next < keys.length ? String(next) : null }
  }
}
