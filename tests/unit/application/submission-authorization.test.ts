...
describe('submission detail authorization', () => {
  ...
  it('lets a submitter read only their own submission within their event', async () => {
    ...
    const own = await service.submit(ownerActor, submitInput())
    ...
    expect(await service.getOwnDetail(ownerActor, own.id)).toMatchObject({ id: own.id })
    expect(await service.getOwnDetail(ownerActor, 'submission-other')).toBeNull()
    expect(await service.getOwnDetail(ownerActor, 'submission-cross-event')).toBeNull()
    expect(await service.getOwnDetail(crossEventActor, 'submission-cross-event')).toMatchObject({
      id: 'submission-cross-event',
    })
    expect(await service.getOwnDetail(ownerActor, 'submission-ghost')).toBeNull()
  })
  ...
  it('keeps the organizer event view separate from submitter-owned retrieval', async () => {
    ...
    expect(await service.getOwnDetail(ownerActor, other.id)).toBeNull()
