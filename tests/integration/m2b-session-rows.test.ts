...
describe('discriminated session rows', () => {
  ...
  it('source wins on redeem: the persisted session derives identity from the token row', async () => {
    ...
    await env.DB.prepare(
      `INSERT INTO submitter_tokens (id, event_id, contact_id, form_id, purpose, token_hash,
                                       expires_at, consumed_at, created_at)
         VALUES ('token-source', ?, 'contact-source', ?, 'cfp', ?, ?, NULL, ?)`,
    )
      .bind(DEMO_CONF_2026_ID, 'f0000000-0000-4000-8000-000000000001', HASH_A, FUTURE, NOW)
      .run()
  ...
  it('source wins on rotate: the persisted rotated session keeps the source identity', async () => {
    await env.DB.prepare('INSERT INTO contacts (id, email, name, created_at) VALUES (?, ?, ?, ?)')
      ...
      .run()
    await env.DB.prepare(
      `INSERT INTO sessions (id, kind, contact_id, event_id, capability, token_hash,
                               expires_at, consumed_at, created_at)
         VALUES ('session-source', 'submitter', 'contact-source', ?, 'cfp', ?, ?, NULL, ?)`,
    )
      .bind(DEMO_CONF_2026_ID, HASH_A, FUTURE, NOW)
      .run()
    const unitOfWork = createSessionUnitOfWork(env.DB)
    ...
    const stored = await env.DB.prepare(
      'SELECT kind, contact_id, event_id, capability FROM sessions WHERE id = ?',
    )
