...
describe('public start endpoint', () => {
  it('POST /api/public/start is an identical generic 202 for existing and new emails with no sensitive fields', async () => {
    ...
    expect(JSON.parse(existingText)).toEqual({
      status: 'accepted',
      guidance:
    ...
    expect(JSON.parse(newEmailText)).toEqual({
      status: 'accepted',
      guidance:
        'Request accepted for this demo. Email delivery is not enabled, so no inbox message will arrive.',
    })
  ...
  it('uses the configured canonical origin even when request host headers are malicious', async () => {
    const response = await app.request(
      'https://attacker.example/api/public/start',
      {
        method: 'POST',
        headers: {
    ...
    const body = await latestCapturedBody(env.DB, 'speaker-a@example.test')
    expect(body).toContain('https://www.openevents.engineer/api/public/session?token=')
...
describe('public session redeem contract', () => {
  ...
  it('sets a Secure cookie over HTTPS and sends same-token replay to CFP recovery', async () => {
    const raw = await startAndReadToken()
  ...
  it.each([
    ...
    ['used', 'used-raw-token'],
  ] as const)('sends an %s token to browser-followable recovery', async (label, raw) => {
    if (label === 'expired') {
    ...
    expect(response.headers.get('location')).toBe(
      label === 'expired' || label === 'used' ? '/start?access=cfp' : '/start?access=invalid',
    )
