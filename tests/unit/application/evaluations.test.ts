...
import {
  ...
  NOW,
  createSubmission,
  createSubmitterActor as createFixtureActor,
  organizerActor,
...
function createSubmitterActor(overrides: Parameters<typeof createFixtureActor>[0] = {}) {
  return createFixtureActor({ ...overrides, capability: 'evaluation' })
}
...
describe('EvaluationService evaluator scoping', () => {
  ...
  it.each(['cfp', 'portal'] as const)(
    'rejects the %s capability before reading the review queue',
    async (capability) => {
      const actor = createFixtureActor({ contactId: REVIEWER_ONE_ID, capability })
