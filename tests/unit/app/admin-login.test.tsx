...
describe('admin login screen', () => {
  it('renders the login form and focuses the secret field', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Use email access' })).toHaveAttribute('href', '/start')
    expect(screen.queryByRole('button', { name: 'Continue with Google or email' })).toBeNull()
    expect(screen.queryByRole('button', { name: /create an organizer account/i })).toBeNull()
  ...
  it('gives a recovery action for a rate-limited organizer login', async () => {
    const user = userEvent.setup()
    fetchHandler = () => jsonResponse({ error: { code: 'forbidden', message: 'Forbidden' } }, 429)
    await mountLogin()
