...
import migration0024Sql from '../../migrations/0024_guard_evaluation_criterion_round_reuse.sql?raw'
import migration0025Sql from '../../migrations/0025_auth_abuse_budgets.sql?raw'
import migration0026Sql from '../../migrations/0026_submitter_token_purpose.sql?raw'
...
import { createCapturedMessageRepository } from '../../src/db'
...
export const MIGRATIONS: D1Migration[] = [
  ...
  {
    name: '0024_guard_evaluation_criterion_round_reuse.sql',
    queries: splitSqlStatements(migration0024Sql),
  },
  {
    name: '0025_auth_abuse_budgets.sql',
    queries: splitSqlStatements(migration0025Sql),
  },
  {
    name: '0026_submitter_token_purpose.sql',
    queries: splitSqlStatements(migration0026Sql),
  },
  {
    name: '0027_email_delivery_outbox.sql',
    queries: splitSqlStatements(migration0027Sql),
  },
  {
    name: '0028_resend_webhook_events.sql',
    queries: splitSqlStatements(migration0028Sql),
  },
  {
...
export const TEST_EMAIL_DELIVERY_CONFIG: import('../../src/application').EmailDeliveryConfig = {
  mode: 'capture',
  payloadKey: {
    keyVersion: 'v1',
    keyMaterialBase64: 'BwcHBwcHBwcHBwcHBwcHBwcHBwcHBwcHBwcHBwcHBwc=',
  },
  environmentKey: 'test',
  payloadRetentionMs: 24 * 60 * 60 * 1000,
}

export async function capturedMessagesForEmail(
  db: D1Database,
  email: string,
): Promise<readonly import('../../src/domain').CapturedMessage[]> {
  return createCapturedMessageRepository(db, TEST_EMAIL_DELIVERY_CONFIG).listByEmail(email)
}

export async function latestCapturedBody(db: D1Database, email: string): Promise<string | null> {
  const messages = await capturedMessagesForEmail(db, email)
  return messages.at(-1)?.body ?? null
}

export function testStartMailBudget(
  suffix = 'one',
  now = FIXED_NOW,
): import('../../src/application').StartMailBudgetReservation {
  const digit = String(suffix.length % 10)
  return {
    operationId: `budget-${suffix}`,
    recipientKey: `v1:start-recipient:${'a'.repeat(63)}${digit}`,
...
/** The complete resettable showcase, layered after the optional programme. */
export async function seedDemoConfShowcase(db: D1Database): Promise<void> {
  await seedDemoConfProgramme(db)
  for (const statement of splitSqlStatements(seedShowcaseSql)) {
    await db.prepare(statement).run()
  }
}
