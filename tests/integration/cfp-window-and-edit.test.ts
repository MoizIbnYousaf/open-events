...
describe('a submitter can edit their own proposal while the call is open', () => {
  ...
  it('never lets one speaker edit another speaker’s proposal', async () => {
    ...
    const strangerCfp = await submitterCookie(env.DB, {}, 'stranger@example.test')
    const strangerDraft = await savePublicDraft(strangerCfp, {
      title: 'Stranger proposal',
      answers: SEEDED_TALK_ANSWERS,
    })
    const strangerSubmit = await app.request(
      '/api/public/submit',
      {
        method: 'POST',
        headers: {
          cookie: cookieHeader(strangerCfp),
          origin: ALLOWED_ORIGIN,
          'content-type': 'application/json',
        },
        body: JSON.stringify({
          originDraftId: strangerDraft,
          formVersionId: DEMO_CONF_2026_VERSION_ID,
          title: 'Stranger proposal',
          answers: SEEDED_TALK_ANSWERS,
          coSpeakers: [],
    ...
    const stranger = parseCookieToken(strangerSubmit.headers.get('set-cookie'))
    if (stranger === null) throw new Error('stranger submit did not elevate the CFP session')
