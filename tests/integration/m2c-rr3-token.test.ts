...
async function insertToken(
  opts: {
    ...
    readonly purpose?: 'cfp' | 'portal' | 'evaluation' | null
  ...
  await env.DB.prepare(
    ...
    .bind(
      ...
      opts.purpose === undefined ? 'cfp' : opts.purpose,
      hash,
...
describe('token issuance with form_id', () => {
  it('the start flow persists form_id on the submitter token row', async () => {
    ...
    expect(response).toMatchObject({ outcome: 'issued' })
    if (response.outcome !== 'issued') throw new Error('expected an issued link')
    const rawToken = new URL(response.accessUrl).searchParams.get('token') ?? ''
...
describe('pre-consume failures leave zero token/session mutation', () => {
  ...
  it('rejects a legacy null-form_id token with 403', async () => {
    const raw = await insertToken({ formId: null, purpose: null })
    await expectDenialWithoutMutation(raw, 'forbidden')
  })
  ...
  it('rejects a row that fails decode at the mapper boundary', () => {
    ...
    expect(() => toSubmitterToken(row)).toThrow(
      new DbDecodeError('CFP or legacy submitter token row is missing form_id'),
    )
  })
})
...
describe('bounded statement counts', () => {
  ...
  it('issueStart executes at most 5 prepared statements', async () => {
    ...
    await unitOfWork.issueStart({
      ...
      budget: testStartMailBudget('counting'),
    })

    // Mail issuance adds one durable outbox row and one capacity reservation
    // to the original bounded batch.
