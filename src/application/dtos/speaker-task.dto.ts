...
export interface SubmissionReadinessDto {
  ...
  /** Outstanding work categories, deduplicated and safe for an aggregate desk. */
  readonly missingTaskKinds?: readonly SpeakerTaskKind[]
  /** Compact non-identity blocker labels used for ranking and filtering. */
...
export function toSubmissionReadinessDto(
  ...
  return {
    ...
    missingTaskKinds: [
      ...new Set(tasks.filter((task) => task.status === 'pending').map((task) => task.kind)),
    ],
    blockers: [
      ...new Set([
        ...tasks.filter((task) => task.status === 'pending').map((task) => task.kind),
        ...(extra.total > extra.completed ? ['speaker_assignment'] : []),
      ]),
    ],
  }
}
