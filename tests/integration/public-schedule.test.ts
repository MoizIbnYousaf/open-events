...
describe('public schedule API', () => {
  ...
  it('keeps uploaded headshots private until a checksum-bound review exists', async () => {
    const storageKey = 'events/demo-conf-2026/contacts/contact-1/headshot/review-pending'
    await env.FILES.put(storageKey, new Uint8Array([137, 80, 78, 71]), {
    ...
    await env.DB.prepare(
      `INSERT INTO uploaded_files
       ...
       VALUES ('pending-headshot', ?, 'contact-1', 'headshot', ?, 'image/png', 4, ?, ?, NULL)`,
    )
      .bind(DEMO_CONF_2026_ID, storageKey, NOW, NOW)
      .run()
    ...
    const body = (await directory.json()) as {
      speakers: Array<{ id: string; hasHeadshot: boolean; photoUrl: string | null }>
    }
    expect(body.speakers.find((speaker) => speaker.id === 'contact-1')).toMatchObject({
      hasHeadshot: false,
      photoUrl: null,
    })

    const publicBytes = await app.request(
      '/api/public/events/demo-conf-2026/speakers/contact-1/headshot',
      undefined,
      bindings(),
    )
    expect(publicBytes.status).toBe(404)
  })

  it('exports exactly the selected still-published sessions', async () => {
    const selected = await app.request(
      '/api/public/events/demo-conf-2026/schedule.ics?ids=submission-1,submission-2,stale',
      undefined,
      bindings(),
    )
    expect(selected.status).toBe(200)
    expect(selected.headers.get('cache-control')).toContain('private')
    const calendar = await selected.text()
    expect(calendar).toContain('UID:submission-1@open-events')
    expect(calendar).not.toContain('submission-2')
    expect(calendar).not.toContain('stale')

    const empty = await app.request(
      '/api/public/events/demo-conf-2026/schedule.ics?ids=',
      undefined,
      bindings(),
    )
