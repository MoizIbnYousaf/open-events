...
describe('public email-link session start', () => {
  ...
  it('shows truthful capture-mode guidance with no link, token, or email echo in the DOM', async () => {
    const user = userEvent.setup()
    renderStartForm()
    ...
    expect(await screen.findByText(/no inbox message will arrive/i)).toBeInTheDocument()
  ...
  it('announces the accepted request from a region that was already mounted', async () => {
    ...
    await waitFor(() => expect(region).toHaveTextContent(/no inbox message will arrive/i))
