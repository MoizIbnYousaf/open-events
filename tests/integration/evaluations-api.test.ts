...
async function submitProposal(
  ...
  return { id: body.id, portalCookie }
}
...
async function issueReviewerAccess(email: string): Promise<{
  readonly cookie: string
  readonly location: string | null
  readonly accessUrl: string
}> {
  const invited = await organizerPost(COMMITTEE_PATH, { email })
  if (invited.status !== 200) throw new Error(`reviewer invite failed with ${invited.status}`)
  const body = (await invited.json()) as { invitePath?: string | null }
  if (body.invitePath === undefined || body.invitePath === null) {
    throw new Error('reviewer invite returned no access URL')
  }
  const accessUrl = new URL(body.invitePath)
  const redeemed = await app.request(
    `${accessUrl.pathname}${accessUrl.search}`,
    undefined,
    bindings(),
  )
  if (redeemed.status !== 303) throw new Error(`reviewer redeem failed with ${redeemed.status}`)
  const cookie = parseCookieToken(redeemed.headers.get('set-cookie'))
  if (cookie === null) throw new Error('reviewer redeem set no session cookie')
  return { cookie, location: redeemed.headers.get('location'), accessUrl: accessUrl.toString() }
}
...
beforeEach(async () => {
  ...
  const cfpCookie = await submitterCookie(env.DB, {}, SPEAKER_EMAIL)
  const submitted = await submitProposal(cfpCookie, 'Workshop proposal')
  submissionId = submitted.id
  speakerCookie = submitted.portalCookie
  reviewerOneCookie = (await issueReviewerAccess(REVIEWER_ONE_EMAIL)).cookie
  reviewerTwoCookie = (await issueReviewerAccess(REVIEWER_TWO_EMAIL)).cookie
})
...
describe('organizer assignments', () => {
  ...
  it('captures a copyable magic link when assigning a never-seen reviewer', async () => {
    ...
    expect(body.invitePath).toMatch(
      /^https:\/\/www\.openevents\.engineer\/api\/public\/session\?token=/,
    )
    expect(await latestCapturedBody(env.DB, 'sbek-reviewer@example.test')).toContain(
      '/api/public/session?token=',
    )
    const inviteUrl = new URL(body.invitePath ?? '')
    const redeemed = await app.request(
    ...
    expect(redeemed.headers.get('location')).toBe('/evaluations')
  })
...
describe('evaluator surface', () => {
  ...
  it('lands an invited reviewer on reviews while generic speaker start stays on the CFP form', async () => {
    async function redeemLocation(email: string): Promise<string | null> {
      ...
      const body = await latestCapturedBody(env.DB, email)
    ...
    const reviewer = await issueReviewerAccess('landing-reviewer@example.test')
    expect(reviewer.location).toBe('/evaluations')
  ...
  it('puts an evaluator on the committee the moment the organizer assigns them', async () => {
    ...
    const invited = await assignReviewer('new.reviewer@example.test')

    // Assignment returns a purpose-bound reviewer invitation. The existing
    // CFP cookie does not silently change roles; redeeming that invitation does.
    expect((await evaluatorGet(outsider)).status).toBe(403)
    const body = (await invited.json()) as { invitePath?: string | null }
    const accessUrl = new URL(body.invitePath ?? '')
    const redeemed = await app.request(
    ...
    const reviewerCookie = parseCookieToken(redeemed.headers.get('set-cookie'))
    const opened = await evaluatorGet(reviewerCookie ?? '')
    expect(opened.status).toBe(200)
  ...
  it('refuses a submission the evaluator was never assigned', async () => {
    const freshCfp = await submitterCookie(env.DB, {}, SPEAKER_EMAIL)
    const otherSubmissionId = (await submitProposal(freshCfp, 'Second proposal')).id
    await assignReviewer(REVIEWER_ONE_EMAIL)
