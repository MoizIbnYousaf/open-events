...
export function runWranglerCount(spawn, binding, dbPath, sql, remote = false) {
...
async function countRows(
  specs,
  { persistTo, remote = false, env = {}, root = process.cwd(), spawn = spawnSync },
) {
  ...
  for (const { key, sql } of specs) {
    counts[key] = runWranglerCount(spawn, BINDING, stateRoot, sql, remote)
  }
  return counts
}
